...
def build_model():
    ...
    # ops.logFile('log_opensees.txt', '-append', '-noEcho')
    ...
    dict_mats, conc_tags, steel_tags, shear_tags, Ec_uc = define_materials()
    ...
    return numStories, Hstory, Htotal, WallElPerStory, ListNodes, ListNodesBasal, ListNodesDrift, \
        IDctrlNode, IDctrlDOF, dict_mats, conc_tags, steel_tags, shear_tags, tags_walls, tags_beams, \
            tags_cols, dict_elems, dict_walls_floor
...
def define_nodes(numStories, Hstory, WallElPerStory, Lw1, Lpas, LineaCol5, Lw2_Yt, Lpas2, LineaCol9,
    ...
    # ListNodesDrift = ListNodesDrift[ListNodesDrift[:, 0].argsort()] # Ordenarlos de forma ascendente
...
def define_materials():
    ...
    dict_mats[matConcCover] = {
        'descr.': 'unconfined concrete',
        'matType': 'Concrete02', 'fpc': fpc_uc, 'epsc0': epc_uc, 'fpcu': fc20_uc, 'epsU': ec20_uc,
        'lambda': 0.1, 'ft': ft, 'Ets':Ets
        }
    ...
    dict_mats[matSteelUnc] = {
        'descr.': 'steel in unconfined concrete', 'matType': 'Hysteretic', 
        'p1': [fy, ey], 'p2': [fu_p, eu_p], 'p3': [fr, er_p],
        'n1': [-fy, -ey], 'n2': [-fu_n, -eu_n], 'n3': [-fr, -er_n],
        'pinchX': 1., 'pinchY': 1., 'damage1': 0., 'damage2': 0.
        }    
    ...
    dict_mats[matConcCoreP2] = {
        'descr.': 'confined concrete in P2',
        'matType': 'Concrete02', 'fpc': fpc_c, 'epsc0': epc_c, 'fpcu': fc20_c, 'epsU': ec20_c,
        'lambda': 0.1, 'ft': ft, 'Ets':Ets
        }
    ...
    fu_p = b * Es * (eu_p - ey) + fy 
    fu_n = b * Es * (eu_n - ey) + fy
    dict_mats[matSteelCoreP2] = {
        'descr.': 'steel in confined concrete in P2', 'matType': 'Hysteretic', 
        'p1': [fy, ey], 'p2': [fu_p, eu_p], 'p3': [fr, er_p],
        'n1': [-fy, -ey], 'n2': [-fu_n, -eu_n], 'n3': [-fr, -er_n],
        'pinchX': 1., 'pinchY': 1., 'damage1': 0., 'damage2': 0.
        }   
    ...
    dict_mats[matConcCoreP3] = {
        'descr.': 'confined concrete in P3',
        'matType': 'Concrete02', 'fpc': fpc_c, 'epsc0': epc_c, 'fpcu': fc20_c, 'epsU': ec20_c,
        'lambda': 0.1, 'ft': ft, 'Ets':Ets
        }
    ...
    fu_p = b * Es * (eu_p - ey) + fy 
    fu_n = b * Es * (eu_n - ey) + fy  
    dict_mats[matSteelCoreP3] = {
        'descr.': 'steel in confined concrete in P3', 'matType': 'Hysteretic', 
        'p1': [fy, ey], 'p2': [fu_p, eu_p], 'p3': [fr, er_p],
        'n1': [-fy, -ey], 'n2': [-fu_n, -eu_n], 'n3': [-fr, -er_n],
        'pinchX': 1., 'pinchY': 1., 'damage1': 0., 'damage2': 0.
        }   
    ...
    dict_mats[matShearP1c01] = {
        'descr.': 'shear material for P1c01', 'matType': 'Elastic', 'E': 0.1 * GAsP1
        } 
    dict_mats[matShearP1c05] = {
        'descr.': 'shear material for P1c05', 'matType': 'Elastic', 'E': 1.0 * GAsP1
        } 
    
    # P2
    AshwebP2 = 1.545  # Gross area of the wall cross section
    GAsP2 = G * AshwebP2  # Shear Stiffness, factor de corte = 1 para sección T
    dict_mats[matShearP2c01] = {
        'descr.': 'shear material for P2c01', 'matType': 'Elastic', 'E': 0.1 * GAsP2
        } 
    dict_mats[matShearP2c05] = {
        'descr.': 'shear material for P2c05', 'matType': 'Elastic', 'E': 1.0 * GAsP2
        } 
    
    # P3
    AshwebP3 = 1.05  # Gross area of the wall cross section
    GAsP3 = 5/6 * G * AshwebP3  # Shear Stiffness  5/6 para sección rectangular
    dict_mats[matShearP3c01] = {
        'descr.': 'shear material for P3c01', 'matType': 'Elastic', 'E': 0.1 * GAsP3
        }     
    dict_mats[matShearP3c05] = {
        'descr.': 'shear material for P3c05', 'matType': 'Elastic', 'E': 1.0 * GAsP3
        } 
    ...
    for matTag in dict_mats:
        mat = dict_mats[matTag]
        if mat['matType'] == 'Concrete02':
            ops.uniaxialMaterial('Concrete02', matTag, mat['fpc'], mat['epsc0'], mat['fpcu'],
                                 mat['epsU'], mat['lambda'], mat['ft'], mat['Ets'])
        elif mat['matType'] == 'Hysteretic':
            ops.uniaxialMaterial('Hysteretic', matTag, 
                                 *mat['p1'], *mat['p2'], *mat['p3'], *mat['n1'], *mat['n2'], *mat['n3'],
                                 mat['pinchX'], mat['pinchY'], mat['damage1'], mat['damage2'])
        elif mat['matType'] == 'Elastic':
            ops.uniaxialMaterial('Elastic', matTag, mat['E'])
    
    return dict_mats, conc_tags, steel_tags, shear_tags, Ec_uc
